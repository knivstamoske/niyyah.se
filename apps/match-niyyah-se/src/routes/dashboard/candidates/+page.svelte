<script lang="ts">
	...
	const STATUS_OPTIONS = [
		{ value: 'all', label: () => m.match_filter_all_statuses() },
		{ value: 'onboarding', label: () => m.match_status_onboarding() },
		{ value: 'verifying', label: () => m.match_status_verifying() },
		{ value: 'active', label: () => m.match_status_active() },
		{ value: 'paused', label: () => m.match_status_paused() },
		{ value: 'matching', label: () => m.match_status_matching() },
		{ value: 'matched', label: () => m.match_status_matched() },
		{ value: 'archived', label: () => m.match_status_archived() },
		{ value: 'banned', label: () => m.match_status_banned() }
	];
	...
	const GENDER_OPTIONS = [
		{ value: 'all', label: () => m.match_filter_all_genders() },
...
<svelte:head>
	<title>{m.match_candidates_title()} | {m.match_portal_title()}</title>
</svelte:head>

<div class="px-4 py-8">
	<!-- Header -->
	<div class="mb-8">
		<h1 class="text-3xl font-bold">{m.match_candidates_title()}</h1>
		<p class="text-slate mt-2">{m.match_candidates_subtitle()}</p>
	</div>
	...
	<div class="bg-white border border-taupe/20 rounded-md p-6 mb-6">
		<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
			<!-- Search -->
			<div class="relative">
				<label for="search" class="block text-sm font-medium mb-2">{m.match_search_label()}</label>
				<div class="relative">
	...
	{#await getCandidates(filters)}
		<div class="text-center py-12">
			<p class="text-slate">{m.match_loading_candidates()}</p>
		</div>
	{:then candidates}
		{#if candidates.length === 0}
			<div class="text-center py-12 bg-cream border border-taupe/20 rounded-md">
				<p class="text-slate">{m.match_no_candidates()}</p>
			</div>
		{:else}
			<div class="bg-white border border-taupe/20 rounded-md overflow-hidden">
				<table class="w-full">
					<thead class="bg-cream border-b border-taupe/20">
						<tr>
							<th class="px-4 py-3 text-left text-sm font-medium">{m.match_table_name()}</th>
							<th class="px-4 py-3 text-left text-sm font-medium hidden md:table-cell"
								>{m.match_table_email()}</th
							>
							<th class="px-4 py-3 text-left text-sm font-medium">{m.match_table_age()}</th>
							<th class="px-4 py-3 text-left text-sm font-medium hidden sm:table-cell"
								>{m.match_table_gender()}</th
							>
							<th class="px-4 py-3 text-left text-sm font-medium hidden md:table-cell"
								>{m.match_table_kommun()}</th
							>
							<th class="px-4 py-3 text-left text-sm font-medium">{m.match_table_status()}</th>
							<th class="px-4 py-3 text-left text-sm font-medium hidden lg:table-cell"
								>{m.match_table_registered()}</th
							>
							<th class="px-4 py-3 text-left text-sm font-medium">{m.match_table_actions()}</th>
						</tr>
					</thead>
	...
	{:catch error}
		<div class="bg-cream border-l-4 border-error p-4">
			<p class="text-sm text-error">{m.match_error_load_candidates()}</p>
		</div>
	{/await}
</div>
