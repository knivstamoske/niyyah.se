...
<svelte:head>
	<title>{m.match_profile_title()} | {m.match_portal_title()}</title>
</svelte:head>
...
<div class="px-4 py-8">
		...
		<div class="flex justify-between items-start">
			<div>
				...
				<p class="text-slate mt-2 flex items-center gap-4 text-sm">
					...
					<span class="flex items-center gap-1">
						<Phone class="w-4 h-4" />
						{profile.phone || m.match_na()}
					</span>
					{#if profile.emailVerified}
						<span class="flex items-center gap-1 text-success">
							<CheckCircle class="w-4 h-4" />
							{m.match_profile_email_verified()}
						</span>
					{/if}
				</p>
			</div>
		...
		<div class="lg:col-span-2 space-y-6">
			...
			<div class="bg-white border border-taupe/20 rounded-md p-6">
				...
				<div class="grid grid-cols-2 gap-6 text-sm">
					...
					<div>
						<div class="text-xs text-slate mb-1">{m.match_profile_nationality()}</div>
						<div class="font-medium text-midnyt">{profile.nationality || m.match_na()}</div>
					</div>
					<div>
					...
					<div>
						<div class="text-xs text-slate mb-1">{m.match_profile_languages()}</div>
			...
			{#if guardian}
				<div class="bg-white border border-taupe/20 rounded-md p-6">
					<h2 class="text-xl font-bold mb-4 flex items-center gap-2">
						...
						{m.match_profile_guardian()}
					</h2>
					<div class="grid grid-cols-2 gap-6 text-sm">
						<div>
							<div class="text-xs text-slate mb-1">{m.match_profile_guardian_name()}</div>
						...
						<div>
							<div class="text-xs text-slate mb-1">{m.match_profile_guardian_phone()}</div>
							<div class="font-medium text-midnyt">{guardian.phone || m.match_na()}</div>
						</div>
						<div>
							<div class="text-xs text-slate mb-1">{m.match_profile_guardian_email()}</div>
							<div class="font-medium text-midnyt">{guardian.email || m.match_na()}</div>
						</div>
					</div>
				</div>
			{/if}
			...
			<div class="bg-white border border-taupe/20 rounded-md p-6">
				<h2 class="text-xl font-bold mb-4 flex items-center gap-2">
					...
					{m.match_profile_activity()}
				</h2>
				{#if events.length === 0}
					<p class="text-slate text-sm italic">{m.match_profile_no_activity()}</p>
				{:else}
					<div class="space-y-4">
		...
		<div class="space-y-6">
			...
			<div class="bg-white border border-taupe/20 rounded-md p-6">
				<h2 class="text-xl font-bold mb-4 flex items-center gap-2">
					...
					{m.match_profile_status_section()}
				</h2>
				<div class="mb-4">
					<div class="text-sm text-slate mb-1">{m.match_profile_current_status()}</div>
					<div class="flex items-center gap-2">
						...
						<button
							...
							title={m.match_profile_change_status()}
						>
				...
				<div class="text-xs text-slate flex items-center gap-1">
					<Clock class="w-3 h-3" />
					{m.match_profile_last_updated({ date: formatDate(profile.updatedAt) })}
				</div>
			</div>
			...
			<div class="bg-white border border-taupe/20 rounded-md p-6">
				<h2 class="text-xl font-bold mb-4 flex items-center gap-2">
					...
					{m.match_profile_dates()}
				</h2>
				<div class="space-y-3">
					<div>
						<div class="text-sm text-slate mb-1">{m.match_profile_registered()}</div>
						<div class="text-sm font-medium text-midnyt">
							{formatDate(profile.userCreatedAt)}
						</div>
					</div>
					<div>
						<div class="text-sm text-slate mb-1">{m.match_profile_created()}</div>
						<div class="text-sm font-medium text-midnyt">
			...
			<div class="bg-white border border-taupe/20 rounded-md p-6">
				<h2 class="text-xl font-bold mb-4 flex items-center gap-2">
					...
					{m.match_profile_messages()}
				</h2>
				{#if messages.length === 0}
					<p class="text-slate text-sm italic">{m.match_profile_no_messages()}</p>
				{:else}
					<div class="space-y-3">
								...
								<div class="flex items-center gap-1.5 text-xs">
									{#if message.readAt}
										...
										<span class="text-success"
											>{m.match_profile_message_read({ date: formatDate(message.readAt) })}</span
										>
									{:else}
										<AlertTriangle class="w-3.5 h-3.5 text-warning" />
										<span class="text-warning">{m.match_profile_message_unread()}</span>
									{/if}
								</div>
							</div>
						{/each}
	...
	{#if isStatusModalOpen}
		...
		>
			...
			>
				<h2 class="text-xl font-bold mb-4">{m.match_modal_change_status()}</h2>

				<div class="mb-4">
					<label for="newStatus" class="block text-sm font-medium mb-2"
						>{m.match_modal_new_status()}</label
					>
					<select
						id="newStatus"
				...
				<div class="mb-6">
					<label for="reason" class="block text-sm font-medium mb-2">{m.match_modal_reason()}</label>
					<textarea
						id="reason"
						bind:value={statusReason}
						rows="3"
						class="w-full px-3 py-2 border border-taupe/30 rounded-md focus:outline-none focus:ring-2 focus:ring-bronze/20 focus:border-bronze"
						placeholder={m.match_modal_reason_placeholder()}
					></textarea>
				</div>
				...
				<div class="flex justify-end gap-3">
					...
					>
						{#if isUpdating}
							...
							></span>
							{m.match_modal_updating()}
						{:else}
							{m.match_modal_update()}
						{/if}
					</button>
				</div>
			</div>
