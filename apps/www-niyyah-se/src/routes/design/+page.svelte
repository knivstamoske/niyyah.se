	...
	<div class="mx-auto max-w-2xl space-y-16">
		...
		<section class="text-center space-y-4">
			...
			<p class="text-lg text-slate max-w-prose mx-auto">
				Defined in <code class="bg-gray-100 px-1 py-0.5 rounded text-sm">libs/ui</code>, driven by
				<span class="font-medium text-midnyt">Organic Minimalism</span>.
			</p>
		</section>
				...
				<div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
					{#each colors as color}
						<div class="flex flex-col group">
							<div
								class="{color.class} h-24 rounded-md flex items-center justify-center mb-2 transition-transform group-hover:-translate-y-1"
							>
			...
			<div class="p-4 space-y-6">
				...
				<div class="space-y-2">
					<p class="leading-relaxed text-midnyt">
						This is regular body text (`text-midnyt`). It should be readable, have a relaxed line
						height, and use the system font stack for clarity. <a
							href="/design"
							class="text-bronze underline decoration-bronze/40 underline-offset-4 hover:decoration-bronze"
							>Links look like this</a
						> and use the bronze accent color.
					</p>

					<blockquote
						class="border-l-4 border-bronze pl-4 py-4 my-4 italic text-midnyt bg-bronze/5 rounded-r-md"
					>
		...
		<section class="space-y-6">
			...
			<div class="space-y-4">
				...
				<div class="flex flex-wrap gap-4 items-center">
					<button
						class="bg-midnyt text-cream px-6 py-2.5 rounded-md hover:bg-midnyt/90 transition-colors font-medium flex items-center gap-2"
					>
					...
					<button
						class="bg-bronze text-cream px-6 py-2.5 rounded-md hover:bg-bronze/90 transition-colors font-medium"
					>
						Accent Action
					</button>

					<button
						class="bg-white border border-slate/30 text-midnyt px-4 py-2.5 rounded-md hover:border-slate/60 transition-colors font-medium"
					>
						<Bell size={20} />
					</button>

					<button
						class="text-midnyt hover:text-bronze transition-colors p-2 hover:bg-taupe/10 rounded-md"
					>
						<User size={24} />
					</button>
					...
					<a
						href="/design"
						class="flex items-center gap-2 text-bronze hover:text-midnyt transition-colors font-medium hover:underline underline-offset-4 decoration-bronze"
					>
			...
			<div class="space-y-4">
				...
				<div class="max-w-sm space-y-4">
					<div>
						...
						<div class="relative">
							<input
								...
								class="w-full pl-10 pr-3 py-2 border border-slate/30 rounded-md focus:outline-none focus:ring-2 focus:ring-bronze/20 focus:border-bronze transition-all"
							/>
					...
					<div>
						<label for="error-input" class="block text-sm font-medium text-midnyt mb-1"
							>Error State</label
						>
						<input
							id="error-input"
							type="text"
							value="Invalid input"
							class="w-full px-3 py-2 border border-error text-error rounded-md focus:outline-none focus:ring-2 focus:ring-error/20"
						/>
					...
					{#if isDropdownOpen}
						...
						>
							<div class="p-1">
								<a
									href="/design"
									class="flex items-center gap-2 px-4 py-2 text-sm text-midnyt hover:bg-cream rounded-md group"
								>
							...
							<div class="p-1">
								<a
									href="/design"
									class="flex items-center gap-2 px-4 py-2 text-sm text-error hover:bg-error/5 rounded-md"
								>
									<LogOut size={16} />
									Sign Out
								</a>
		...
		<section class="space-y-6">
			...
			<div class="space-y-4">
				...
				<div
					class="bg-white border border-taupe/20 rounded-md p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
				>
					<div class="space-y-1">
				...
				<div class="bg-cream border-l-4 border-bronze p-4 flex gap-3 items-start">
					...
					<div>
						...
						<p class="text-slate text-xs mt-1">
							You have 3 pending action items to complete before you can start matching.
						</p>
					</div>
				</div>
			</div>
			...
			<div class="space-y-4">
				...
				<div class="border border-slate/10 rounded-md divide-y divide-slate/10 bg-white">
					<!-- List Item 1 -->
					<div
						class="p-4 flex items-center justify-between hover:bg-cream/30 transition-colors cursor-pointer group"
					>
						<div class="flex items-center gap-3">
							<div
								class="h-10 w-10 rounded-full bg-taupe/10 flex items-center justify-center text-taupe group-hover:bg-taupe/20 transition-colors"
							>
								<User size={20} />
							</div>
							<div>
					...
					>
						<div class="flex items-center gap-3">
							<div
								class="h-10 w-10 rounded-full bg-success/10 flex items-center justify-center text-success group-hover:bg-success/20 transition-colors"
							>
								<CheckCircle size={20} />
							</div>
							<div>
			...
			<div class="space-y-4">
				...
				<div class="flex gap-8">
					...
					<button
						class="flex items-center gap-2 text-sm font-medium text-midnyt hover:text-bronze transition-colors"
					>
					...
					<button
						class="flex items-center gap-2 text-xs font-medium bg-white border border-slate/20 rounded-full px-3 py-1.5 text-midnyt hover:border-slate/40 transition-colors"
					>
					...
					>
						...
						<button
							class="px-3 py-1.5 bg-white text-slate hover:text-midnyt hover:bg-cream/50 transition-colors"
							>SV</button
						>
						<div class="w-px bg-slate/20 h-full"></div>
						<button
