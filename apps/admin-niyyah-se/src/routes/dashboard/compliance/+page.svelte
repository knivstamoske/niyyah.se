...
<svelte:head>
	<title>{m.admin_compliance_title()} | {m.admin_portal_title()}</title>
</svelte:head>

<div class="px-4 py-8">
	<!-- Header -->
	<div class="mb-8">
		<h1 class="text-3xl font-bold">{m.admin_compliance_title()}</h1>
		<p class="text-slate mt-2">{m.admin_compliance_subtitle()}</p>
	</div>
	...
	<div class="bg-white border border-taupe/20 rounded-md p-6">
		<h2 class="text-xl font-bold mb-6 flex items-center gap-2">
			...
			{m.admin_compliance_banned_section()}
		</h2>

		{#if banned.length === 0}
			<div class="text-center py-12 flex flex-col items-center gap-2">
				<p class="text-slate">{m.admin_compliance_no_banned()}</p>
			</div>
		{:else}
			<div class="mb-4 text-sm text-slate">
				{m.admin_compliance_banned_count({
					count: banned.length,
				...
				{#each banned as candidate (candidate.id)}
					<div class="bg-cream/50 border border-taupe/10 rounded-md p-4">
						...
						<div class="grid grid-cols-4 gap-4 mb-4 text-sm">
							...
							<div>
								<div class="text-xs text-slate mb-1">{m.admin_compliance_banned_on()}</div>
							...
							>
								{#if deletingIds.has(candidate.id)}
									<span
										class="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin"
									></span>
									{m.admin_action_deleting()}
								{:else}
									<Trash2 class="w-3.5 h-3.5" />
									{m.admin_action_delete_gdpr()}
								{/if}
							</button>
						</div>
					</div>
