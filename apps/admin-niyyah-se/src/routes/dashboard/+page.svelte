<script lang="ts">
	...
	const STATUS_LABELS: Record<string, () => string> = {
		onboarding: () => m.admin_status_onboarding(),
		verifying: () => m.admin_status_verifying(),
		active: () => m.admin_status_active(),
		paused: () => m.admin_status_paused(),
		matched: () => m.admin_status_matched(),
		archived: () => m.admin_status_archived(),
		banned: () => m.admin_status_banned()
	};
	...
	function formatRelativeTime(date: Date): string {
		...
		if (diffSec < 60) return m.admin_activity_time_ago({ time: 'just now' });
		if (diffMin < 60) return m.admin_activity_time_ago({ time: `${diffMin}m` });
		if (diffHour < 24) return m.admin_activity_time_ago({ time: `${diffHour}h` });
		return m.admin_activity_time_ago({ time: `${diffDay}d` });
	}
...
<svelte:head>
	<title>{m.admin_dashboard_title()} | {m.admin_portal_title()}</title>
</svelte:head>
...
<div class="px-4 py-8">
	...
	<div class="mb-8">
		...
		<p class="text-slate mt-2">{m.admin_dashboard_subtitle()}</p>
	</div>

	{#await getDashboardMetrics()}
		<div class="text-center py-12">
			<p class="text-slate">{m.admin_loading_metrics()}</p>
		</div>
	{:then metrics}
		...
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
			...
			<div class="bg-white border border-taupe/20 rounded-md p-6 relative overflow-hidden">
				...
				<div class="relative z-10">
					<div class="text-sm text-slate mb-1">{m.admin_metric_total_candidates()}</div>
					<div class="text-3xl font-bold text-midnyt">{metrics.totalCandidates}</div>
					<div class="text-xs text-slate mt-2">
						{m.admin_metric_new_registrations({ count: metrics.newRegistrations30d })}
					</div>
				</div>
			</div>
			...
			>
				...
				<div class="relative z-10">
					<div class="text-sm text-slate mb-1">{m.admin_metric_pending_verifications()}</div>
					<div class="text-3xl font-bold text-warning">{metrics.pendingVerifications}</div>
					<div class="text-xs text-slate mt-2">{m.admin_metric_requires_attention()}</div>
				</div>
			</a>
			...
			<div class="bg-white border border-taupe/20 rounded-md p-6 relative overflow-hidden">
				...
				<div class="relative z-10">
					<div class="text-sm text-slate mb-1">{m.admin_metric_active_candidates()}</div>
					<div class="text-3xl font-bold text-success">{metrics.activeCandidates}</div>
					<div class="text-xs text-slate mt-2">{m.admin_metric_ready_for_matching()}</div>
				</div>
			</div>
			...
			<div class="bg-white border border-taupe/20 rounded-md p-6 relative overflow-hidden">
				...
				<div class="relative z-10">
					<div class="text-sm text-slate mb-1">{m.admin_metric_paused_candidates()}</div>
					<div class="text-3xl font-bold text-slate">{metrics.pausedCandidates}</div>
					<div class="text-xs text-slate mt-2">{m.admin_metric_temporarily_inactive()}</div>
				</div>
			</div>
		</div>
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
			...
			<div class="bg-white border border-taupe/20 rounded-md p-6">
				<h2 class="text-xl font-bold mb-4">{m.admin_chart_lifecycle()}</h2>
				<div class="space-y-3">
			...
			<!-- Gender Distribution -->
			<div class="bg-white border border-taupe/20 rounded-md p-6">
				<h2 class="text-xl font-bold mb-4">{m.admin_chart_gender_distribution()}</h2>
				<div class="space-y-4">
					<!-- Male -->
					<div>
						<div class="flex justify-between text-sm mb-1">
		...
		<div class="bg-white border border-taupe/20 rounded-md p-6 mb-8">
			<h2 class="text-xl font-bold mb-4">{m.admin_chart_geographic_distribution()}</h2>
			<div class="space-y-3">
	...
	{:catch error}
		<div class="bg-cream border-l-4 border-error p-4 mb-8">
			<p class="text-sm text-error">{m.admin_error_load_metrics()}</p>
		</div>
	{/await}
	...
	<div class="bg-white border border-taupe/20 rounded-md p-6">
		<div class="flex items-center gap-2 mb-4">
			...
			<h2 class="text-xl font-bold">{m.admin_activity_title()}</h2>
		</div>
		...
		{:then activities}
			{#if activities.length === 0}
				<p class="text-slate text-center py-8">{m.admin_activity_empty()}</p>
			{:else}
				<div class="space-y-3">
